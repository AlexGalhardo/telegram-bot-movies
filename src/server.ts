...
async function fetchMoviesByGenre(genreId: number, page: number = 1): Promise<TMDBMovie[]> {
  const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&with_genres=${genreId}&language=pt-BR&sort_by=vote_average.desc&vote_count.gte=500&page=${page}`);
  const data = await res.json();
  
  const movies: TMDBMovie[] = await Promise.all(data.results.map(async (movie: any) => {
    const resDetail = await fetch(`https://api.themoviedb.org/3/movie/${movie.id}?api_key=${TMDB_API_KEY}&language=pt-BR`);
...
async function getAvailableMoviesForUser(genreId: number, userId: number): Promise<SavedMovie[]> {
  const availableMovies = savedMovies.filter(movie => 
    movie.genre_ids.includes(genreId) && 
    !isMovieAlreadyRecommended(movie.id, userId)
  );
...
async function sendMovieRecommendations(chatId: number, genreName: string) {
  ...
  try {
    ...
    if (availableMovies.length === 0) {
      return bot.sendMessage(chatId, `Desculpe, não encontrei novos filmes de ${genreName} para recomendar no momento.`);
    }
    ...
    await markMoviesAsRecommended(selectedMovies.map(m => m.id), chatId);
